 ...
 * - Goal progress (positions vs targets)
 * - What it needs from you (if anything)
 * - Emails you a summary + action items
 */
...
import { sendWeeklyReportEmail, sendActionRequired } from "./email-client.js";
import { KEYWORD_GOALS, SERVICE_GOALS, OVERALL_GOALS } from "./seo-goals.js";
...
${page2.slice(0, 8).map((k) => `- "${k.query}" (pos ${k.position}, ${k.impressions} impressions/mo)`).join("\n")}`
  ...
  // Cross-reference GSC data with our keyword goals
  const gscKeywordMap = new Map(page2.map((k) => [k.query.toLowerCase(), k.position]));
  const topPageKeywordMap = new Map(
    (topPagesResult.status === "fulfilled" ? topPagesResult.value : []).map((p) => [p.page, p.position])
  );

  const highPriorityGoals = KEYWORD_GOALS.filter((g) => g.targetPosition <= 3 && g.monthlySearchVolume === "high");
  const goalsSection = `## 🎯 Goal Progress — Keyword Rankings
| Keyword | Target | Status |
|---------|--------|--------|
${highPriorityGoals.map((g) => {
    const pos = gscKeywordMap.get(g.keyword.toLowerCase());
    const status = pos === undefined
      ? "📊 _No data yet_"
      : pos <= g.targetPosition
        ? `✅ Position **${pos}** (goal: ${g.targetPosition})`
        : `🔴 Position **${pos}** (goal: ${g.targetPosition} — ${Math.round(pos - g.targetPosition)} away)`;
    return `| "${g.keyword}" | Top ${g.targetPosition} | ${status} |`;
  }).join("\n")}

**Monthly Lead Targets:**
${SERVICE_GOALS.map((s) => `- ${s.service}: target **${s.targetLeadsPerMonth} leads/month** from \`${s.primaryKeyword}\``).join("\n")}
> Current site-wide conversion count: **${ga4?.totalConversions ?? "_unavailable_"}**`;

  // ── What needs human input ──────────────────────────────────────────────
  const needsInput: string[] = [];
  if (!stats) needsInput.push("GSC credentials not working — check `GSC_SERVICE_ACCOUNT_JSON` secret");
  if (!ga4) needsInput.push("GA4 not connected — check `GA4_PROPERTY_ID` and `GSC_SERVICE_ACCOUNT_JSON` secrets");
  if (recentPosts.length === 0) needsInput.push("Blog agent produced no posts this week — check `ANTHROPIC_API_KEY` and `OPENAI_API_KEY` secrets");
  if (page2.length > 5) needsInput.push(`${page2.length} keywords stuck on page 2 — top opportunities: ${page2.slice(0, 3).map(k => `"${k.query}"`).join(", ")}`);
  if (ga4 && ga4.totalConversions < OVERALL_GOALS.monthlyOrganicLeads / 4) {
    needsInput.push(`Conversions this week (${ga4.totalConversions}) are below pace for monthly target of ${OVERALL_GOALS.monthlyOrganicLeads} — CTA audit needed`);
  }
  if (noLeadsPages.length > 0) {
    needsInput.push(`${noLeadsPages.length} high-traffic pages still have zero conversions — SEO agent will fix CTAs this week`);
  }

  const needsSection = needsInput.length > 0
    ? `## ⚠️ Needs Your Attention\n${needsInput.map((n) => `- [ ] ${n}`).join("\n")}`
    : `## ✅ Nothing Needs Your Attention\nAll agents ran successfully. All systems green.`;
...
_Auto-generated by Innovate Digital Agent System • ${weekOf}_`;
  ...
  // ── Send email ───────────────────────────────────────────────────────────
  await sendWeeklyReportEmail(body, weekOf);
  if (needsInput.length > 0) {
    await sendActionRequired(needsInput);
  }
}
